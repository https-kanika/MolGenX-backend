    ...
    def forward(self, x, target_features, affinity=None, latent_z=None):
        ...
        Args:
            ...
            affinity (torch.Tensor, optional): Binding affinity values.
                                            Shape: [batch_size, 1]
            latent_z (torch.Tensor, optional): Latent vector from VAE.
        ...
        # Now process through target encoder
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            z = torch.randn((inputs.size(0), 64), device=device)
            protein_features = protein_encoder(protein_sequences)
            outputs = model(inputs, protein_features, affinities, latent_z=z)
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} [Val]"):
                ...
                # Generate latent vector for validation (add this line)
                z, mu, logvar = vae_encoder(protein_sequences)
                
                protein_features = protein_encoder(protein_sequences)
                # Add latent_z parameter here
                outputs = model(inputs, protein_features, affinities, latent_z=z)
...
class ProteinVAEEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        h = torch.cat([hidden[-2], hidden[-1]], dim=1)  # last layer, both directions
        mu = self.fc_mu(h)
